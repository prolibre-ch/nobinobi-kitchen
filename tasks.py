...
def coverage(c):
    ...
    """
    c.run("coverage run --source nobinobi_kitchen runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/nobinobi-kitchen.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ nobinobi_kitchen")
...
def lint(c):
    ...
    """
    c.run("flake8 nobinobi_kitchen tests")
...
def release(c, bumpsize=''):
    ...
    import nobinobi_kitchen
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=nobinobi_kitchen.__version__))
    c.run("git push --tags")
